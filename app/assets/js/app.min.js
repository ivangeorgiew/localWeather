...
var getJSON = function getJSON(url, cb) {
  ...
  xhr.onload = function () {
    ...
    if (status >= 200 && status <= 400) cb(xhr.response);else console.log(status);
  };
  xhr.send();
};

var getLocation = function getLocation(ipData) {
  document.querySelector('.h2--city').innerHTML = ipData.city;
  document.querySelector('.h2--country').innerHTML = ipData.country;
};

var getWeather = function getWeather(ipData) {
  getJSON('http://api.openweathermap.org/data/2.5/weather?lat=' + ipData.loc.split(',')[0] + '&lon=' + ipData.loc.split(',')[1] + '&units=imperial' + '&appid=174ca7c117e1e592e6796b14b9395aff', function (wData) {
    ...
    var wVar = wImg[wData.weather[0].main];

    document.querySelector('.h2--temp').innerHTML = Math.round(wData.main.temp) + ' °F';
    document.querySelector('body').style.background = 'url(' + wVar[1] + ') no-repeat center center fixed';
...
var convert = function convert() {
  var button = document.querySelector('.b--convert');
...
getJSON('http://ipinfo.io/?callback', function (ipData) {
  getLocation(ipData);
  getWeather(ipData);
});

document.querySelector('.b--convert').onclick = function () {
  convert();
};